...
class HeartPatientFeatures(BaseModel):
    ...
    trestbps: int = Field(..., ge=TRESTBPS_MIN, le=TRESTBPS_MAX, description=f"Pressão arterial em repouso [{TRESTBPS_MIN}-{TRESTBPS_MAX}]")
    chol: int = Field(..., ge=CHOL_MIN, le=CHOL_MAX, description=f"Colesterol sérico [{CHOL_MIN}-{CHOL_MAX}]")
    thalach: int = Field(..., ge=THALACH_MIN, le=THALACH_MAX, description=f"Frequência cardíaca máxima atingida [{THALACH_MIN}-{THALACH_MAX}]")
